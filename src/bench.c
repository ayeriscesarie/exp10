...
row_result_t measure_v3_avx2(int bench_n, int repeats) {
    row_result_t rr;
    memset(&rr, 0, sizeof(rr));
    ...
    /* accuracy: compare AVX2 output to scalar V */
    {
        exp10_v3_avx2_kernel(in, out, N);
...
row_result_t measure_sleef_avx2(int bench_n, int repeats) {
    ...
    /* accuracy: compare AVX2 output to scalar V */
    {
        exp10_sleef_avx2_kernel(in, out, N);
